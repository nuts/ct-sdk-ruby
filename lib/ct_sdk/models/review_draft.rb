...
module CtSdk
  class ReviewDraft
    ...
    # If set, this value must be unique among reviews. For x-annotation-example, if you want to have only one review per customer and per product, you can set the value to `customer's id` and `product's id`.
    ...
    def self.openapi_types
      {
        :'key' => :'String',
        :'uniqueness_value' => :'String',
        :'locale' => :'String',
        :'author_name' => :'String',
        :'title' => :'String',
        :'text' => :'String',
        :'target' => :'OneOfProductResourceIdentifierChannelResourceIdentifier',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CtSdk::ReviewDraft` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CtSdk::ReviewDraft`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@rating.nil? && @rating > 100
        invalid_properties.push('invalid value for "rating", must be smaller than or equal to 100.')
      end

      if !@rating.nil? && @rating < -100
        invalid_properties.push('invalid value for "rating", must be greater than or equal to -100.')
      end
    ...
    def valid?
      return false if !@rating.nil? && @rating > 100
      return false if !@rating.nil? && @rating < -100
      true
    end
    ...
    def rating=(rating)
      ...
      if !rating.nil? && rating < -100
        fail ArgumentError, 'invalid value for "rating", must be greater than or equal to -100.'
      end
