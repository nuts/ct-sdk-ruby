  ...
  class ProductDraft
    ...
    def self.openapi_types
      {
        ...
        :'key' => :'String',
        :'description' => :'Object',
        :'categories' => :'Array<CategoryResourceIdentifier>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CtSdk::ProductDraft` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CtSdk::ProductDraft`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [product_type, name, slug, key, description, categories, category_order_hints, meta_title, meta_description, meta_keywords, master_variant, variants, tax_category, search_keywords, state, publish].hash
    end
